...
void my_exit(int sig)
{
    ...
    send(socket_rece, (char*)&headdata, sizeof(struct Data),0);
...
int main()
{
    ...
    //视频
    addr_host.sin_port = htons(8000);//将一个无符号短整型的主机数值转换为网络字节顺序，即大尾顺序(big-endian)
    ...
    // if(bind(socket_rece, (struct sockaddr *)&addr, sizeof(struct sockaddr)) < 0){
    //     cout<<"server connect error"<<endl;
    // }

    // headdata.dwDataType = 21;  //21表示解码端
    // headdata.dwBufSize = 0;
    // send(socket_rece, (char*)&headdata, sizeof(struct Data), MSG_WAITALL);
    string register_identity = "soft_video \r\n";
    ...
    while(true)
    {
        ...
        {
            cout<<headdata.dwDataType<<"ok"<<endl;
        }
    }
}