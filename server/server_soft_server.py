...
recv_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM) #tcp，链接服务器

class test_udp(Thread):
    def __init__(self):
        super(test_udp,self).__init__()
        global recv_socket
        host = socket.gethostname()
        recv_socket.bind((host,62223))
    def run(self):
        global recv_socket
        global socket_dict
        now_sec = 0
        now_usec = 0
        while True:
            data, addr = recv_socket.recvfrom(65000)
            print(len(data))
            if "soft_video" in socket_dict.keys(): #解码端在的话才转发，否则就抛弃掉
                fhead = data[0:24]
                data_type, length,sec,usec= struct.unpack('IILL', fhead)
                print(data_type,length,sec,usec)
                print(len(data[24:]))
                if now_sec< sec:
                    socket_dict["soft_video"].request.sendall(fhead[0:8])
                    socket_dict["soft_video"].request.sendall(data[24:])
                    now_sec = sec
                    now_usec = usec
                elif now_sec == sec and now_usec<= usec:
                    socket_dict["soft_video"].request.sendall(fhead[0:8])
                    socket_dict["soft_video"].request.sendall(data[24:])
                    now_sec = sec
                    now_usec = usec
    ...
    def join(cls, client):
        ...
        with cls.lock:
            if client.method == "robot_client":
                ...
                socket_dict[client.method].close()#关闭上一个链接
...
class UserHandler(socketserver.BaseRequestHandler):
    def handle(self):
        ...
        print(f'Connected: {self.client}')
        try:
            self.process_command()
        except Exception as e:
            print(e)
        finally:
            print(f'Closed: {self.client}  {self.identity}')
